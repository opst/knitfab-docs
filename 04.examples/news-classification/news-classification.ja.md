# 例:Knitfab での LLM のファインチューニングによるニュース分類 <!-- FIXME: 全体的に# による見出し階層がおかしい。spam-filter と同様に修正すること。-->
...
<!-- 寺田文例 >> -->
本事例では、ニュース記事の分類タスクのためのLLMのファインチューニングを題材にし
て、Knitfab による履歴管理の自動化の効果を示します。一般にファインチューニングで
は、学習データやハイパーパラメータを変化させながら、モデルの訓練と性能確認を繰り
返します。その間の履歴記録をKnitfabに任せることで、開発者の負担を大きく軽減でき
ます。
<!-- 寺田文例 << -->
...
**コンポーネント:<!-- FIXME: テキスト全体にこれら **...** 見出しを #（シャープ）のネスティングによる見出しに変更すること-->
...
### タスク
- [例:Knitfab での LLM のファインチューニングによるニュース分類 ](#例knitfab-での-llm-のファインチューニングによるニュース分類-)
