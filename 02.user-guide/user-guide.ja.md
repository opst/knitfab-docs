...
ワークフローエンジンは、ユーザの定義したプランにしたがって、必要なタスクを自動的に実行します。
自動リネージ管理機能は、ワークフローエンジンがタスクを遂行する際に、その各タスクと入出力の関係を追跡することで Knitfab が生成した全データの生成履歴を自動的に記録します。
...
このとき、 `KNIT_ID` の部分には、パージしたい "データ" の Knit ID を指定してください。
